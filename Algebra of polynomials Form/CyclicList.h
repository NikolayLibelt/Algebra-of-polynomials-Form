...
public:
	...
	CyclicList multiply(CyclicList list,int maxDegree, int count) {
		CyclicList result;
		Monomial* current = head->getNext();
		while (current != head) {
			Monomial* newCurrent = list.head->getNext();
			while (newCurrent != list.head) {
				result.add(current->multiply(*newCurrent, maxDegree, count));
